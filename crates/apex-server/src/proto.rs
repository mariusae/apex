...
pub enum ClientMsg {
    ...
    /// Notify a window (`MetaOp::Notify`): its handle shows it, the
    /// session's does while any window is notified, and a click on the
    /// session's takes the user to the oldest.
    Notify { window: WindowId },
